...
BEGIN
    ...
    RAISE NOTICE 'Loaded silver.crm_cust_info in % seconds',
    ...
    t_end := clock_timestamp();
    RAISE NOTICE 'Loaded silver.crm_prd_info in % seconds',
        ROUND(EXTRACT(EPOCH FROM (t_end - t_start))::numeric, 2);
    ...
    SELECT
        ...
        CASE WHEN sls_order_dt = 0 OR LENGTH(sls_order_dt::text) != 8
             THEN NULL ELSE sls_order_dt::text::date END,
        CASE WHEN sls_ship_dt = 0 OR LENGTH(sls_ship_dt::text) != 8
             THEN NULL ELSE sls_ship_dt::text::date END,
        CASE WHEN sls_due_dt = 0 OR LENGTH(sls_due_dt::text) != 8
             THEN NULL ELSE sls_due_dt::text::date END,
        CASE
    ...
    t_end := clock_timestamp();
    RAISE NOTICE 'Loaded silver.crm_sales_details in % seconds',
        ROUND(EXTRACT(EPOCH FROM (t_end - t_start))::numeric, 2);
